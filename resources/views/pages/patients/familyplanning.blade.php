<x-app-layout>
                ...
                <div class="card-inner card-inner-lg">
                    <div class="nk-block-head nk-block-head-lg pb-3">
                        <div class="nk-block-between">
                            <div class="nk-block-head-content">
                                <h4 class="nk-block-title text-2xl fw-bold">Family Planning </h4>
                                <div class="nk-block-des">
                                    <p>Patient information base on the Family Planning .</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="nk-block">
                        <div class="nk-data data-list">
                            <div class="data-head">
                                <h6 class="overline-title">Family Planning  Records</h6>
                            </div>
                            @include('pages.patients.familyplans.sub.familyplanlist')
                        </div>
                    </div>
                </div>
        ...
        <div class="modal-dialog modal-xl">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <p>This form is for Family Planning .</p>
                    <hr class="mt-2 mb-2">
                    <form action="{{ route('family.save') }}" id="family_form" method="POST" autocomplete="off">
                        @csrf
                        @include('pages.patients.familyplans.sub.familyplan')
                    </form>
                </div>
            </div>
        </div>
    ...
    <script>
        function edit_famplan(id, date, method, remarks) {

            const form = document.getElementById('family_form');

            const date_val = document.getElementById('inp_date');
            const method_val = document.getElementById('inp_moc');
            const remarks_val = document.getElementById('inp_remarks');

            date_val.value = date;
            method_val.value = method;
            remarks_val.value = remarks;

            form.action = "/prenatal/family/update/" + id
        }
    </script>
